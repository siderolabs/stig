...
DATA=$($TALOSCTL get staticpod kube-apiserver -o yaml | grep -c tls-cipher-suites)
if [ $DATA -ne 1 ]; then
   echo "kube-apiserver does not have tls-cipher-suites set"
   exit 1
fi

DATA=$($TALOSCTL get staticpod kube-apiserver -o yaml | grep tls-cipher-suites | cut -f2 -d'=')
