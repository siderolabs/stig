...
#DATA=$($TALOSCTL get staticpod kube-apiserver -o yaml |grep -c '\--request-timeout')
#if [ $DATA -ne 1 ]; then
#   echo "kube-apiserver does not have request-timeout set"
#   exit 1
#fi

DATA=$($TALOSCTL get staticpod kube-apiserver -o yaml |grep '\--request-timeout' | cut -f2 -d'=')
if [ "$DATA" == "0" ]; then
